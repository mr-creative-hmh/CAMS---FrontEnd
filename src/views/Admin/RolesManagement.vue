      ...
      >
        <template v-slot:top>
          <v-toolbar color="grey-darken-3" rounded flat>
            <v-toolbar-title
              ><v-icon>mdi-account-key</v-icon> Roles
            ...
            <v-dialog v-model="dialog" max-width="500px">
              ...
              <v-card>
                <v-toolbar
                  :color="formTitle == 'New Role' ? 'primary' : 'info'"
                  :title="formTitle"
                ></v-toolbar>
                <v-card-text>
                  <v-container>
...
function editRole(item) {
  HTTP.put("/role/" + item.ID, { name: item.RoleName })
    .then((response) => {
      auth.$patch({
        showAlert: true,
    ...
    .catch((error) => {
      ...
      auth.$patch({
        showAlert: true,
        message:
          "An error occurred while editing the role. Please try again later.",
        isError: true,
      });
    });
}
...
function createRole(item) {
  HTTP.post("/role/", { name: item.RoleName })
    .then((response) => {
      auth.$patch({
        showAlert: true,
        message: response.data.Message,
