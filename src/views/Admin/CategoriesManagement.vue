      ...
      >
        <template v-slot:top>
          <v-toolbar color="grey-darken-3" rounded flat>
            <v-toolbar-title
              ><v-icon>mdi-order-bool-descending</v-icon> Categories
            ...
            <v-dialog v-model="dialog" max-width="500px">
              ...
              <v-card>
                <v-toolbar
                  :color="formTitle == 'New Category' ? 'primary' : 'info'"
                  :title="formTitle"
                ></v-toolbar>
                <v-card-text>
                  <v-container>
            ...
            <v-dialog v-model="dialogDelete" max-width="500px">
              <v-card>
                <v-toolbar color="red" title="Delete Category"></v-toolbar>
...
function editCategory(item) {
  HTTP.put("/category/" + item.ID, { name: item.Name })
    .then((response) => {
      auth.$patch({
        showAlert: true,
...
function createCategory(item) {
  ...
  HTTP.post("/category/", { name: item.Name })
    .then((response) => {
      auth.$patch({
        showAlert: true,
        message: response.data.Message,
...
function deleteItemConfirm() {
  deleteCategory(editedItem.value);
