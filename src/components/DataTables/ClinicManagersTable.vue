      ...
      >
        <template v-slot:top>
          <v-toolbar color="grey-darken-3" rounded flat>
            <v-toolbar-title
              ><v-icon>mdi-account-tie</v-icon> Clinic Managers
            ...
            <v-dialog v-model="dialog" max-width="500px">
              ...
              <v-card>
                <v-toolbar
                  :color="
                    formTitle == 'New Clinic Manager' ? 'primary' : 'info'
                  "
                  :title="formTitle"
                ></v-toolbar>
                <v-card-text>
...
function fetchclinicmanagers() {
  HTTP.get("/clinicmanager")
    .then((response) => {
      clinicmanagers.value = response.data.data;
      console.log(clinicmanagers.value);
      console.log("clinicmanagers :", response.data.data);
    })
    .catch((error) => {
      console.error("Error fetching clinicm anager:", error);
    });
}
...
function editClinicMan(item) {
  const clinic = item.Clinic.ID ? item.Clinic.ID : item.Clinic;
  HTTP.put("/clinicmanager/" + item.ID, {
    name: item.User.Name,
    ...
    .catch((error) => {
      console.error("Error edit clinic manager:", error);
      auth.$patch({
        showAlert: true,
        message:
          "An error occurred while editing the clinic manager. Please try again later.",
        isError: true,
      });
    });
}

async function createClinicMan(item) {
  try {
    console.log("clinic manager : ", item);
    const response = await HTTP.post("/clinicmanager", {
      name: item.User.Name,
...
function deleteClinicMan(item) {
  HTTP.delete("/clinicmanager/" + item.ID)
    .then((response) => {
      auth.$patch({
        showAlert: true,
        message: response.data.Message,
    ...
    .catch((error) => {
      console.error("Error delete clinic manager:", error);
      auth.$patch({
        showAlert: true,
        message:
          "An error occurred while deleting the clinic manager. Please try again later.",
        isError: true,
      });
    });
}
...
function changepass(item) {
  editedIndex.value = clinicmanagers.value.indexOf(item);
...
function deleteItemConfirm() {
  deleteClinicMan(editedItem.value);
  clinicmanagers.value.splice(editedIndex.value, 1);
  initialize();
  closeDelete();
}
function close() {
...
function save() {
  if (editedIndex.value > -1) {
    //Edit
    editClinicMan(editedItem.value);
    Object.assign(clinicmanagers.value[editedIndex.value], editedItem.value);
  } else {
    //Create
    createClinicMan(editedItem.value);
    clinicmanagers.value.push(editedItem.value);
  }
  if (!auth.isError) {
    initialize();
    close();
